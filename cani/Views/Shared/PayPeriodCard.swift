...
struct PayPeriodCard: View {
    ...
    /// Solde maximum parmi toutes les périodes affichées — sert à normaliser la barre de progression.
    ...
    private var barFraction: CGFloat {
        ...
        let ratio = period.projectedBalance / maxBalance
        // Clamp entre 0.03 (toujours visible) et 1
        let clamped = max(Decimal(0.03), min(ratio, Decimal(1)))
        return CGFloat(NSDecimalNumber(decimal: clamped).doubleValue)
    }

    private var barColor: Color { period.isTight ? amberColor : .green }
